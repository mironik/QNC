...
fn plane(start: u32, size: vec2<u32>, uv: vec2<f32>) -> f32 {
    let pos = uv * vec2<f32>(size) - vec2<f32>(0.5);
    let base = vec2<i32>(floor(pos));
    let weight = fract(pos);
    let top = mix(sample_at(start, size, base), sample_at(start, size, base + vec2<i32>(1, 0)), weight.x);
    let bottom = mix(sample_at(start, size, base + vec2<i32>(0, 1)), sample_at(start, size, base + vec2<i32>(1, 1)), weight.x);
...
fn convert(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    let uv = (vec2<f32>(id.xy) + vec2<f32>(0.5)) / vec2<f32>(f32(spec.dw), f32(spec.dh));
    ...
    let y = (plane(0u, vec2<u32>(spec.sw, spec.sh), uv) - y_bias) / y_range;
    let u = (plane(spec.u_start, vec2<u32>(spec.cw, spec.ch), uv) - 128.0 * scale) / c_range;
    let v = (plane(spec.v_start, vec2<u32>(spec.cw, spec.ch), uv) - 128.0 * scale) / c_range;
    let kr = 0.2126;
    let kb = 0.0722;
    let kg = 1.0 - kr - kb;
