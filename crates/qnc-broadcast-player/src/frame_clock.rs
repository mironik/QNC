...
impl FrameClockConfig {
    ...
    pub fn frame_interval_ticks(self) -> Option<ClockTick> {
        if self.rate.rate_num == 0 || self.rate.rate_den == 0 {
            return None;
        }
        let numerator = CLOCK_UNITS_PER_BASE_UNIT
            .checked_mul(u128::try_from(self.timebase.fps_den).ok()?)?
            .checked_mul(u128::from(self.rate.rate_den))?;
        let denominator = u128::try_from(self.timebase.fps_num)
            .ok()?
            .checked_mul(self.rate.magnitude_num())?;
        if numerator == 0 {
            return None;
        }
    ...
    fn due_slots_at(self, elapsed_ticks: ClockTick) -> ClockTick {
        if self.rate.rate_num == 0 || self.timebase.fps_num <= 0 || self.timebase.fps_den <= 0 {
            return 0;
        }
        let numerator = elapsed_ticks
            .saturating_mul(self.timebase.fps_num as u128)
            .saturating_mul(self.rate.magnitude_num());
        let denominator = CLOCK_UNITS_PER_BASE_UNIT
            .saturating_mul(self.timebase.fps_den as u128)
...
mod tests {
    ...
    #[test]
    fn fractional_source_timebase_has_no_integer_fps_drift() {
        for (num, den) in [(24000, 1001), (30000, 1001), (60000, 1001)] {
            let config =
                FrameClockConfig::new(Timebase::new(num, den).unwrap(), FrameClockRate::normal());
            let mut clock = FrameClock::start(config, 0, 0);
            for frame in 0..=1200u64 {
                let due = (u128::from(frame) * 1_000_000_000 * den as u128).div_ceil(num as u128);
                if due > 0 {
                    assert_eq!(clock.next_due_frame(due - 1), None);
                }
                assert_eq!(clock.next_due_frame(due).unwrap().frame, frame);
            }
        }
    }

    #[test]
    fn invalid_deserialized_clock_cannot_produce_frames() {
        for (num, den) in [(0, 1), (25, 0), (-25, 1), (25, -1)] {
            ...
            let mut clock = FrameClock::start(config, 0, 0);
            assert_eq!(config.frame_interval_ticks(), None);
            assert_eq!(clock.next_due_frame(10_000_000_000), None);
        }
    }
    ...
    fn normal_config(frame_rate_num: u32) -> FrameClockConfig {
        FrameClockConfig::new(
            Timebase::new(i64::from(frame_rate_num), 1).unwrap(),
            FrameClockRate::normal(),
        )
    }
}
