...
pub trait VideoDecodeAdapter {
    /// Prefer a cheap, shared immutable payload handle for the bounded replay cache.
    type VideoFrame: Clone;

    /// Stage repositioning; decode/IO belongs to later preparation, never to Play.
    fn cue_video(&mut self, _request: EngineFrameRequest) -> Result<(), BroadcastEngineError> {
        Err(BroadcastEngineError::new(
            BroadcastEngineErrorKind::Contract,
            "video cue not supported",
        ))
    }
    ...
    /// A nonblocking adapter returns NotReady while its existing packet queue is empty.
...
pub trait AudioOutputAdapter {
    type AudioPacket: Clone;

    fn cue_audio(&mut self, _request: EngineFrameRequest) -> Result<(), BroadcastEngineError> {
        Err(BroadcastEngineError::new(
            BroadcastEngineErrorKind::Contract,
            "audio cue not supported",
        ))
    }
    ...
    /// Nonblocking polling of the exact sample range for this carrier frame.
    /// NotReady leaves the audio cursor unchanged; video preparation is independent.
    ...
    /// Queue/commit while paused must remain inaudible and must not start the device.
    ...
    /// Nonblocking start of an already opened device and committed queue; no decoding/opening.
    fn start_audio(&mut self) -> Result<Vec<BroadcastEvent>, BroadcastEngineError>;
    /// Silence/flush queued output but keep the opened device and decoder for resume/cue.
...
pub trait FramePresenter {
    ...
    /// Open/prepare presentation resources before playback readiness, never on first Play.
    fn prepare_presentation(
        &mut self,
        source: &EngineSourceHandle,
    ) -> Result<Vec<BroadcastEvent>, BroadcastEngineError>;

    /// Prepare/check the anchor image without displaying it. Pending GPU work returns false.
    /// Called during idle preparation only, never from ready Play.
    fn prepare_start_frame(
        &mut self,
        frame: &DecodedVideoFrame<Self::VideoFrame>,
    ) -> Result<bool, BroadcastEngineError>;

    fn reset_presentation(&mut self) -> Result<Vec<BroadcastEvent>, BroadcastEngineError> {
        Ok(Vec::new())
    }
...
/// One frame-atomic playout handoff.
///
/// Audio and video for the same program frame cross this boundary together.
/// Output adapters may target a workstation monitor, SDI hardware or a network
/// sender without changing transport or playlist decode.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct PlayoutFrame<V, A> {
    pub frame: FrameNumber,
    pub video: Option<DecodedVideoFrame<V>>,
    pub audio: Option<AudioFramePacket<A>>,
}

pub trait PlayoutOutput {
    type VideoFrame: Clone;
    type AudioPacket: Clone;

    fn cue_output(&mut self, _request: EngineFrameRequest) -> Result<(), BroadcastEngineError> {
        Err(BroadcastEngineError::new(
            BroadcastEngineErrorKind::Contract,
            "output cue not supported",
        ))
    }
    ...
    /// Append while running: do not reset the queue, commit preroll or restart output.
    ...
    /// Start only the already prepared output. Commit must not start paused output.
    fn start_playout(&mut self) -> Result<Vec<BroadcastEvent>, BroadcastEngineError>;

    /// Pause transport output without releasing prepared decode resources.
