...
//! Fixture text only. Live `data/diagnostics/player.log` is not engine input.
...
12 AV_F session=s generation=1 sequence=50 frame=50 gpu_dma_us=1
...
12 AV_F session=s generation=1 sequence=90 frame=90 gpu_dma_us=1
";
    let points = parse_player_log_av_points(log);
