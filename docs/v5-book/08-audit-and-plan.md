...
### Izvedeno (devetnaesto odobrenje): izvršitelj uvoza

`qnc-ingest-import-worker`: `claim_next` → plan iz `IngestWorkPlan` (link po `playback.input`; kopija proxyja; kopija originala) → izvorni medij kroz `qnc-media-stream` (lokalno ili udaljeno) → kopija u komadima kroz `.partial` i preimenovanje u `original/` ili `proxy/` projekta → `finish_import`. Piše isključivo kroz `ContentWriteTransport` (proširen: `queue_selected`, `claim_next`, `finish_import`). Radnja „Uvezi” u `qnc-ingest-application` stavlja odabrane u red i pokreće uvoz u pozadini. **Nije izvedeno:** generiranje proxyja, kopija postera (traži `thumbnail_uri` u `FinishImport`), test kroz cijelu aplikaciju i na stvarnoj kartici, nastavak klipa zaglavljenog u `Processing`. **Granica LAN/intranet:** izvor se čita kroz mrežni transport, ali kopija se piše u direktorij projekta stroja na kojem radi izvršitelj; bez lokalnog pristupa projektu uvoz se odbija s porukom.
