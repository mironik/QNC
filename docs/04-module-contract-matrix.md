...
| scanner | source.scan.roles | camera catalog, source transport, root scope, registrirani IndexReader adapteri | potvrdene grupe, blocked/unresolved stavke i evidence kroz javni rezultat | read-only; caller-owned rezultat, nema DB writea | app/store/UI, ugradjeni brand switch, Media Probe, Filmstrip, Wave, proxy kao zaseban clip; docs/29 |
| Source Reference Contract | source.reference.validate, source.reference.resolve | source URI i relativne reference | jedinstveni SourceReference i wire modeli, provjera granice roota | cisti podaci, bez I/O | filesystem, network, DB, app workflow; docs/29 |
| Source Grouping | source.groups.assemble | eksplicitne original/proxy/related veze i dostupnost datoteka | source/root scoped grupe i sukobi; nije clip/probe DB | cista funkcija, bez I/O, clip ID-a ili writea | parser, scanner, transport I/O, probe, DB, app workflow; docs/29 |
| Source Index Contract | source.index.validate | verzionirane grupe i vec ocitane file cinjenice | validirani batch / zapis / receipt | cisti podaci, bez I/O | filesystem, network, DB, scanner, probe, app workflow; docs/30 |
| Source Index DB Adapter | source.index.write, source.index.read | DB URI, owner transport binding/ovlast, validirani batch ili record ID | trajni odnosi original/proxy/support i idempotentni receipt | zasebna owner baza; isti lokalni i mrezni ugovor | scanner, camera parser, probe, media obrada, app/UI workflow; docs/30 |
| JSON Transport | transport.json.request, transport.json.respond | resolver, endpoint, ograniceni JSON ugovor, read/write ovlast | ograniceni JSON odgovor | stateless; nema DB sheme niti workflowa | SQL, DB write, scanner, probe, app registry; docs/31 |
| Media Record Contract | media.records.validate | javni source zapis, metadata snapshot, dokumenti dokaza | binding, phase/completeness/report, pravila nepromjenjivih revizija | cisti podaci bez I/O i mergea | filesystem, transport, DB write, probe, app workflow; docs/31 |
| Media Record DB Adapter | media.records.write, media.records.read, media.evidence.read, media.acquisition.begin/finish/read | DB URI, owner binding/ovlast, snapshot ili javni ID | trajni camera/final snapshoti, XML/JSON dokazi i jedinstveni acquisition pokusaj po media URI-ju | zasebna owner baza; isti local/LAN/intranet ugovor | scanner, camera parser, media I/O, probe, app/UI workflow; docs/31 i docs/33 |
| Camera Pattern Catalog Reader | camera.patterns.read | catalog QNC URI, resolver/access konfiguracija | provjereni public DB snapshot, verzija, obrasci, evidence i coverage gaps | read-only; SQLite ili isti JSON kroz storage endpoint | detector, scanner, probe, DB write, app workflow; docs/28 |
| camera detector | source.camera.detect | catalog snapshot, source transport, eksplicitni root scope i limiti | recording root i role kandidati, dvosmislenosti i greske; nije konacni clip katalog | call-local listing cache; nema writeova | probe, XML parser, clip import, poslovne odluke aplikacije; docs/28 |
| Media Metadata Contract | media.metadata.validate | versioned clip zapis, odvojeni original/proxy streamovi i field evidence | missing/invalid field report; nema I/O | stateless, no DB writes | scanner, probe, media access, DB write, app workflow; docs/25 |
